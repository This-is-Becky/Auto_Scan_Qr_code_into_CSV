...
class ModernQRScanner(ctk.CTk):
    ...
    def update_camera(self):
        ...
        if self.switch_var.get() == "on" and self.cap:
            ...
            if ret:
                ...
                for barcode in decode(frame):
                    ...
                    # Only update label if we aren't showing a "Saved" or "Error" message
                ...
                # If no QR is seen and the lock has expired, reset to "Ready"
                    # --- ADD THIS LOGIC ---
                if not found_qr:
                    self.current_data = None  # This wipes the "memory"
                    # If the message lock (the "Saved" notification) has expired,
                    # we clear the data because it's no longer in front of the lens.
                    if current_time > self.message_lock_until:
            ...
            self.video_label.configure(image=None, text="Camera Paused")
            if current_time > self.message_lock_until:
